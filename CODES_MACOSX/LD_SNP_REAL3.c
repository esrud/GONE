...
double Hom[II], EHom[II], Nsnp[II], Fi[II], Nind[SS], Qsnp[SS], f, fp;
...
*/		}
	...
	// CALCULATION OF F FOR EACH INDIVIDUAL
	...
	{
		Nind[s] += 1.0;
		if (snp[i][s][1] == 1) Qsnp[s] += 1.0;
		if (snp[i][s][2] == 1) Qsnp[s] += 1.0;
	}
	for (s=1; s<NSNP; s++)
	{
		Qsnp[s] = Qsnp[s] / (2.0*Nind[s]);
		if ((tracelevel != 0)&&(s<10))	fprintf(fout, "%d Nind=%f Qsnp=%f\n", s, Nind[s], Qsnp[s]);
	}
	...
	{
		Nsnp[i] += 1.0;
		if ( ((snp[i][s][1] == 1) && (snp[i][s][2] == 1)) || ((snp[i][s][1] == 2) && (snp[i][s][2] == 2)) )
		Hom[i] += 1.0;
		EHom[i] += 1.0 - (2.0*Qsnp[s]*(1.0-Qsnp[s]));
	}

	for (i=1; i<=NIND; i++)
	{
		Fi[i] = (Hom[i]-EHom[i]) / (Nsnp[i]-EHom[i]);
		if (tracelevel != 0)	fprintf(fout, "IND %d  Hom=%f EHom=%f NSNPS=%f Fi=%f\n", i, Hom[i], EHom[i], Nsnp[i], Fi[i]);
		f += Fi[i] / (double)NIND;
	}
	fprintf(fout, "\n\n");
