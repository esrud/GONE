...
//		else		d2_[b] = ( (D2_[b] / (4.0*VV_[b])) );
	...
	fprintf(fpar," NIND(real sample)=%d\n NSNP=%d\n NSNP_calculations=%d\n NSNP_+2alleles=%d\n NSNP_zeroes=%d\n NSNP_monomorphic=%d\n NIND_corrected=%f\n freq_MAF=%f\n F_dev_HW=%f\n G_var_bet_ind=%f\n\n",
			NIND, NSNP, numSNP, snp_m2, snp_mZ, snp_mono, 1.0/accmean(&AVE_numNIND), freq_MAF, accmean(&P_p2)/accmean(&pq), accmean(&E_p2)/accmean(&pq));
